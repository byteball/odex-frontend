...
const PortfolioPanel = (props: *) => {
  const { accountAddress, accountExplorerUrl, transactions, exchangeAddress, tokenData } = props;
    ...
    <Box>
      <Box py={0}>
        ...
        <TextDiv py={2} small muted>
          Here is your Obyte address that is linked to the exchange. Your deposits should come from this address and
          your withdrawals will be also sent to this address.
        </TextDiv>
        <FlexRow py={2} px={0} fontSize={Fonts.FONT_SIZE_SMALL}>
          <a href={accountExplorerUrl} target="_blank" rel="noopener noreferrer" className="bp3-text-overflow-ellipsis">
            {accountAddress}
          </a>
      ...
      <Box my={2}>
        <TransactionsTable transactions={transactions} accountAddress={accountAddress} tokenData={tokenData} />
      </Box>
    </Box>
  );
};
  ...
  return (
    <Spring from={{ opacity: 0 }} to={{ opacity: 1 }}>
      {props => (
        <Box style={props}>
          <Text muted>
            Add a token that is not listed among the default ODEX tokens. If the token has not yet been added to the
            ODEX you can also list the token.
            <br />
          ...
          <Flex py={3}>
            ...
            <Button intent="primary" text="Search Token" minimal onClick={handleDetectToken} />
            <Box pl={2} style={{ display: 'flex', alignItems: 'center' }}>
              <Help position={Position.LEFT}>This button will detect whether the entered asset exists.</Help>
            </Box>
          </Flex>
          {tokenSymbol && (
            <Box>
                  ...
                  <Flex py={1} width="50%">
                    ...
                    <Box pl={2} pt={1}>
                      <Help position={Position.RIGHT}>
                        ...
                        <br />
                        <br />
                        If another person wants to trade {tokenSymbol} too, that person needs to add this token on their
                        own account to discover the corresponding markets.
                        <br />
                        <br />
                      </Help>
                    </Box>
                  ...
                  <Flex py={1} width="50%">
                    ...
                    <Box pl={2} pt={1}>
                      <Help position={Position.RIGHT}>
                        ...
                        If another person wants to trade {tokenSymbol}, that person needs to add this token on their own
                        account to discover the corresponding markets.
                        <br />
                        <br />
                      </Help>
                    </Box>
...
const PremiumListingPanel = (props: *) => {
  return <Box>For premium listing, contact us at support@proofsuite.com</Box>;
};
...
const AuthorizationsPanel = (props: *) => {
  ...
  const message = !isValidAddress(address)
    ? 'Please input a valid address'
    : address === accountAddress
    ? `You can't authorize your wallet address`
    : `You can't authorize the address that you authorized in the past`;
