      ...
      <g>
        ...
        <foreignObject x={cx - boxWidth / 2} y={cy - boxHight / 2} width={boxWidth} height={boxHight}>
          <div xmlns="http://www.w3.org/1999/xhtml" style={{width: '100%', height: '100%', display: 'flex', justifyContent: 'center', flexDirection: 'column', fontSize: boxFontSize }}>
            <p style={{margin: '0'}}>{payload.balance}</p>
            <p style={{margin: '0', wordWrap: 'break-word'}}>{payload.symbol}</p>
          </div>
        </foreignObject>
        ...
        <text x={ex + (cos >= 0 ? 1 : -1) * 4} y={ey} style={{"fontSize": 12}} textAnchor={textAnchor} fill={Colors.WHITE}>{`${value} ${payload.currency}`}</text>
        <text x={ex + (cos >= 0 ? 1 : -1) * 4} y={ey} dy={18} style={{"fontSize": 12}} textAnchor={textAnchor} fill={Colors.WHITE}>
